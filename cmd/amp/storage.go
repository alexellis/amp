...
var (
	...
	storagePutCmd = &cobra.Command{
		...
		Long: `Put command (amp kv put) creates a storage object with the key-value input
		if the key does not already exist. Else, it updates the existing key with the new input value.`,
	...
	storageGetCmd = &cobra.Command{
		...
		Long: `Get command (amp kv get) retrieves a key-value pair based on the input key.
	...
	storageDeleteCmd = &cobra.Command{
		...
		Long: `Delete command (amp kv del or amp kv rm) deletes the key-value pair
		in storage based on the specified key. If not found, an appropriate message is returned.`,
		Aliases: []string{"rm"},
		RunE: func(cmd *cobra.Command, args []string) error {
